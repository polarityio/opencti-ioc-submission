...
// OpenCTI-specific GraphQL error codes
const OPENCTI_ERROR_CODES = new Set([
  'AUTH_REQUIRED',
  'FORBIDDEN',
  'VALIDATION_ERROR',
...
/**
 * Checks if an error response contains OpenCTI-specific AUTH_REQUIRED errors
 * @param {Object} error - Error object that may contain GraphQL errors
 * @returns {Boolean} - True if AUTH_REQUIRED error detected
 */
const isAuthRequiredError = (error) => {
  if (!error || !error.body || !error.body.errors || !Array.isArray(error.body.errors)) {
    return false;
  }
  
  return error.body.errors.some(err => 
    err.extensions && 
    err.extensions.code === 'AUTH_REQUIRED'
  );
};

/**
 * Checks if an error response contains OpenCTI GraphQL errors
 * @param {Object} error - Error object that may contain GraphQL errors
 * @returns {Boolean} - True if GraphQL errors detected
 */
...
/**
 * Extracts OpenCTI error details from GraphQL error response
 * @param {Object} error - Error object containing GraphQL errors
 * @returns {Object} - Categorized error information
 */
const parseOpenCTIError = (error) => {
  if (!isGraphQLError(error) || error.body.errors.length === 0) {
    return null;
  }
  ...
  if (errorCode === 'VALIDATION_ERROR' || errorCode === 'GRAPHQL_VALIDATION_FAILED') {
    return {
